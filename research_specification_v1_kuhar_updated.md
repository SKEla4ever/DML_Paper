...
- Remaining uncertainty：以上 validation uncertainty 固定 model seed `20260615`；下一步单独运行 model-seed sensitivity，完成后才执行第一次 locked test evaluation；
...
- HHAR split sensitivity：已完成 3 个预注册 split seeds（`20260615`、`20260616`、`20260617`）；正式 hyperparameter selection 必须 aggregate 三个 frozen splits 的 validation metrics；model-seed sensitivity 与 split-seed sensitivity 分开报告，不将两种 variance 混为一个标准差；
- HHAR FedAvg references：primary high-budget 为 BatchNorm + SGD momentum `0.9` + constant learning rate `0.01` + 1 local epoch + 50 rounds；communication-efficient supplementary 为 BatchNorm + SGD momentum `0.9` + constant learning rate `0.03` + 1 local epoch + 20 rounds；后续方法仅在 matching budget 下比较；
...
8. 对 frozen HHAR FedAvg primary setting 运行独立 model-seed sensitivity，之后才执行 locked test evaluation；
