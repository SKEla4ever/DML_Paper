...
- Diagnostic：tiny-overfit 与 random-label controls 通过，centralized learning 明显有效，real/IID FedAvg 在相同 communication 下呈现可解释差异；validation 对 centralized、real FedAvg 和 IID FedAvg 均系统性难于 test。后续预注册 split-seed sensitivity 已完成，因此 algorithm tuning 不依赖单一 split 作方法选择。

#### HHAR Split-Seed Sensitivity V1（2026-07-11）

- Pre-registration：固定 split seeds `20260615`、`20260616`、`20260617`，model/optimizer seed 固定为 `20260615`；只有 synchronized-execution split 改变，architecture、optimizer、training budget、client count 和 communication accounting 均保持不变；
- Stability rule：validation 和 test 的 sample Macro-F1 SD 均不超过 `0.05`，且 range 均不超过 `0.10`；ranking consistency 与 absolute-score stability 分开判断；
- Controls：三个 splits 的 tiny-overfit Macro-F1 均为 `1.0000`，random-label validation Macro-F1 为 `0.0724`、`0.1088`、`0.1700`；
- Real user-device FedAvg：validation `0.3879 +/- 0.0084`、test `0.5741 +/- 0.0143`，通过预注册 stability thresholds；
- IID-client FedAvg：validation `0.5245 +/- 0.0082`、test `0.7681 +/- 0.0073`，通过预注册 stability thresholds；三个 splits 上 IID 均优于 real-client FedAvg，validation margin 为 `0.1262` 至 `0.1425`，test margin 为 `0.1890` 至 `0.2025`；
- Centralized control：validation `0.5683 +/- 0.0579`、range `0.1030`，未通过 stability thresholds；test `0.8705 +/- 0.0248`、range `0.0439`；
- Split difficulty：test-minus-validation mean gap 对 centralized、real FedAvg 和 IID FedAvg 分别为 `0.3021`、`0.1862` 和 `0.2435`，说明 validation 较难是 systematic split-difficulty difference，而非单一 seed failure；
- Device condition：8 个 devices 中 7 个通过 stability thresholds；预先标记的 weak-device `s3mini_2` 为 `0.3467 +/- 0.0661`、range `0.1300`，必须单独报告，不与 adequate-support devices 合并解释；
- Selection rule：所有后续 HHAR hyperparameter selection 必须在这 3 个 frozen splits 上 aggregate validation metrics，test 不参与 selection；model-seed sensitivity 单独运行和报告，不与 split variance 合并；
- Frozen report 与 machine-readable summary：`hhar_delivery/reports/hhar/hhar_split_seed_sensitivity_v1.md` 和 `hhar_delivery/reports/hhar/hhar_split_seed_sensitivity_v1_summary.json`。

#### HHAR FedAvg 3-Split Validation-Only Tuning V1（2026-07-11）

- Selection boundary：所有 tuning histories 只生成 `train` 和 `validation` performance metrics；test prediction、test metric 和 test-driven checkpoint selection 均未运行；global validation Macro-F1 是唯一 primary selection metric，pair-level metrics 仅作 descriptive context；
- Pilot grid：在固定 model seed `20260615`、20 rounds、1 local epoch、full participation 下，对 8 个 BatchNorm/GroupNorm、Adam/SGD 和 learning-rate configs 运行三个 frozen splits；
- Pilot winner：BatchNorm + SGD momentum `0.9` + constant learning rate `0.03`，validation Macro-F1 为 `0.5396 +/- 0.0163`，range `0.0306`，communication 为 `267,565,440` bytes；practical-tie set 仅包含该配置；
- Local computation：2 local epochs 的 validation mean 为 `0.5226`，低于 1 local epoch 的 `0.5396`，且 worst split 下降 `0.0123`，因此冻结 1 local epoch；
- Long-horizon diagnostic：将 pilot winner 原样延长至 50 rounds 后，三个 splits 均下降，validation mean 从 round 20 的 `0.5396` 降至 `0.3910`；独立 rerun 的 round-20 checkpoints 与 pilot 逐值相同，说明下降不是 environment drift；
- Target-budget refinement：在 50 rounds 下比较 V1 pilot top-3，constant SGD learning rate `0.01` 以 `0.4767 +/- 0.0095` 胜出；Adam `0.0003` 为 `0.4139 +/- 0.0183`，SGD `0.03` 为 `0.3910 +/- 0.0278`；
- Bounded LR-schedule check：step decay `0.03 -> 0.003` 为 `0.4472 +/- 0.0131`，cosine decay `0.03 -> 0.001` 为 `0.4567 +/- 0.0140`，均低于 constant `0.01`；预注册 stop rule 已触发，不再增加 HHAR FedAvg tuning candidates；
- Frozen primary high-budget reference：BatchNorm、SGD momentum `0.9`、constant learning rate `0.01`、1 local epoch、50 rounds、full participation；validation `0.4767 +/- 0.0095`，range `0.0190`，communication `668,913,600` bytes；
- Frozen communication-efficient supplementary reference：BatchNorm、SGD momentum `0.9`、constant learning rate `0.03`、1 local epoch、20 rounds、full participation；validation `0.5396 +/- 0.0163`，communication `267,565,440` bytes；
- Budget rule：20-round 和 50-round references 不作 unequal-budget absolute-score comparison；后续 algorithms 必须在 matching round/communication budget 下比较；
- Model-seed follow-up：上述 tuning estimate 固定 model seed `20260615`；后续已完成独立 `3 splits x 5 model seeds` validation-only characterization，以及单独 pre-register 的 evaluation-only locked test；
- Final report 与 machine-readable summary：`hhar_delivery/reports/hhar/hhar_fedavg_lr_schedule_v1.md` 和 `hhar_delivery/reports/hhar/hhar_fedavg_lr_schedule_v1_summary.json`。

#### HHAR FedAvg Model-Seed Sensitivity V1（2026-07-16）

- Design：冻结三个 split seeds 和 FedAvg primary high-budget configuration；model seed `20260615` 为 development seed，`20260616` 至 `20260619` 为 confirmatory seeds；形成完整 `3 x 5` crossed validation-only matrix；
- Conditional expansion：初始三个 model-seed marginals 的 sample SD 为 `0.0511`，超过预注册上限 `0.05`，range `0.0969` 仍低于 `0.10`；因此按预注册规则加入 seeds `20260618` 和 `20260619`，未改动任何 hyperparameter、round 或 communication budget；
- Final global result：五个 model-seed marginal means 为 `0.4767`、`0.4004`、`0.4973`、`0.4941` 和 `0.4538`；balanced validation Macro-F1 为 `0.4645`，model-seed marginal sample SD 为 `0.0398`，range 为 `0.0969`，通过预注册 stability rule；
- Confirmatory estimate：排除 development seed 后，四个 confirmatory model-seed marginals 为 `0.4614 +/- 0.0452`，比 development marginal 低 `0.0153`；
- Separate uncertainty：三个 split-seed marginals 的 sample SD 为 `0.0075`、range 为 `0.0143`；不使用 pooled 15-cell SD，以免混淆 model-seed 与 split-seed uncertainty；
- Descriptive decomposition：model seed、split seed 和 interaction 分别占 balanced matrix total sum of squares 的 `96.1%`、`2.9%` 和 `1.0%`；该分解无 replicated cells，不作 inferential p-value interpretation；
- Interpretation：虽然 global threshold 通过，model-seed range 仍接近预注册上限，且 device-level seed sensitivity 对 5/8 devices 超过同一 descriptive threshold；后续 HHAR headline comparisons 必须保留五-seed distribution，不能依赖单次 seed ranking；
- Locked-test boundary：15 个 final checkpoints 均在 test access 前冻结并记录 SHA-256；随后已按单独 pre-registration evaluation-only 加载全部 15 个 checkpoints，未执行 retraining、checkpoint selection 或 seed selection；
- Final report 与 machine-readable summary：`hhar_delivery/reports/hhar/hhar_fedavg_model_seed_expansion_v1.md` 和 `hhar_delivery/reports/hhar/hhar_fedavg_model_seed_expansion_v1_summary.json`。

#### HHAR FedAvg Locked Test V1（2026-07-16）

- Boundary：这是 frozen HHAR FedAvg baseline 第一次且最终的 locked test；在任何 test prediction 生成前，已固定 15 个 checkpoint SHA-256、三个 split identities、evaluation code、primary/secondary analyses、physical-user bootstrap 和 stop rule；
- Execution：完整运行 `3 split seeds x 5 model seeds` 的 15 个 registered cells，共生成 94,560 条 window-level predictions；全部为 inference-only，未执行 retraining、test-driven tuning、checkpoint selection 或 seed selection；
- Global result：balanced test Macro-F1 为 `0.6347`，balanced Accuracy 为 `0.6282`；五个 model-seed marginals 的 sample SD/range 为 `0.0180 / 0.0465`，三个 split-seed marginals 为 `0.0182 / 0.0362`，两类 uncertainty 分开报告；
- Confirmatory estimate：排除 development seed `20260615` 后，四个 confirmatory model-seed marginals 为 `0.6376 +/- 0.0193`，相对 development marginal 为 `+0.0148`；
- Validation-to-test shift：balanced test-minus-validation Macro-F1 为 `+0.1702`，与 setup diagnostic 中 validation 系统性更难的观察一致，因此不解释为 training improvement；
- Descriptive decomposition：model seed、split seed 和 interaction 分别占 test matrix total sum of squares 的 `53.6%`、`45.8%` 和 `0.7%`；无 replicated cells，不报告 inferential p-values；
- Activity/device context：`stand` 的 model-seed marginal SD 最高，为 `0.0794`；预先标记的 weak-device `s3mini_2` supported-class Macro-F1 为 `0.3613`，且每个 cell 平均仅 `80.7` 个 windows、4 个 supported classes，必须继续单列；
- Physical-user boundary：9 名 physical users 的 balanced supported-class Macro-F1 mean 为 `0.6126`，user bootstrap 95% CI 为 `[0.4029, 0.8033]`；每名用户 test support 仅覆盖 1 至 3 个 activities，因此该结果仅作 descriptive supported-class analysis，不是 six-class fairness estimate；
- Closed-test rule：本结果现作为 HHAR FedAvg frozen test reference；禁止根据该 test performance 继续调整 FedAvg hyperparameters、选择 checkpoint 或选择 seed。后续 algorithms 只能使用 matching budget、matching split/model-seed policy 和 validation-only selection；
- Final report 与 machine-readable summary：`hhar_delivery/reports/hhar/hhar_fedavg_locked_test_v1.md` 和 `hhar_delivery/reports/hhar/hhar_fedavg_locked_test_v1_summary.json`。
...
- HHAR split/model sensitivity：已完成 3 个预注册 split seeds（`20260615`、`20260616`、`20260617`）和 5 个 model seeds（`20260615` 至 `20260619`）的完整 crossed characterization；正式 comparison 必须保留两个因素并分别报告，不将其合并为 pooled SD；
- HHAR FedAvg references：primary high-budget 为 BatchNorm + SGD momentum `0.9` + constant learning rate `0.01` + 1 local epoch + 50 rounds，其 `3 x 5` balanced validation/test Macro-F1 为 `0.4645 / 0.6347`，test model-seed 和 split-seed marginal SD 分别为 `0.0180` 和 `0.0182`；communication-efficient supplementary 为 BatchNorm + SGD momentum `0.9` + constant learning rate `0.03` + 1 local epoch + 20 rounds；后续方法仅在 matching budget、matching split/model-seed policy 和 validation-only selection 下比较；
...
6. **已完成（2026-07-11）：** 完成 HHAR 3-seed split sensitivity，冻结 split identities、stability thresholds、multi-split validation selection rule 与 weak-device reporting rule；
7. **已完成（2026-07-11）：** 完成 HHAR FedAvg 3-split validation-only tuning、50-round target-budget refinement 与 bounded LR-schedule check，冻结 20/50-round references、matching-budget rule 和 tuning stop rule；
8. **已完成（2026-07-16）：** 对 frozen HHAR FedAvg primary setting 完成 `3 splits x 5 model seeds` validation-only characterization，冻结全部 15 个 checkpoints、separate variance reporting 和无 seed selection rule；
9. **已完成（2026-07-16）：** 在 test access 前单独 pre-register HHAR FedAvg locked test，evaluation-only 评估全部 15 个 frozen checkpoints 并发布所有 cells；test 现已关闭，不再用于 FedAvg tuning 或 selection；
10. 为每个 RQ 预先定义最终 table/figure、target Macro-F1、normalized communication budgets 和 statistical comparison；
11. 冻结 FedRep split point、Ditto regularization search space、fixed local computation budget 及统一调参预算；
12. 写出完整的 UCI HAR smoke-test protocol 和 CAPTURE-24 primary experiment protocol，包括 input sampling rate、seeds、participation rate、stopping rule、validation rule 和 resource cap；
13. 完成以上 specification 工作后再开始实现，不在实现过程中随意扩大研究范围。
