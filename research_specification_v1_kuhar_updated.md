...
7. 为每个 RQ 预先定义最终 table/figure、target Macro-F1、normalized communication budgets 和 statistical comparison；
8. 冻结 1D-CNN、normalization 选择、FedRep split point、Ditto regularization search space、fixed local computation budget 及统一调参预算；
9. 写出完整的 UCI HAR smoke-test protocol 和 CAPTURE-24 primary experiment protocol，包括 input sampling rate、seeds、participation rate、stopping rule、validation rule 和 resource cap；
10. 完成以上 specification 工作后再开始实现，不在实现过程中随意扩大研究范围。
