...
- Model-seed follow-up：上述 tuning estimate 固定 model seed `20260615`；后续已完成独立 `3 splits x 5 model seeds` validation-only characterization，第一次 locked test 仍保持锁定，等待单独 pre-registration；
...
- Locked-test boundary：15 个 final checkpoints 均已冻结并记录 SHA-256；test performance 尚未生成。下一阶段必须单独 pre-register，并 evaluation-only 加载全部 15 个 checkpoints，禁止 retraining 和 seed selection；
...
- HHAR FedAvg references：primary high-budget 为 BatchNorm + SGD momentum `0.9` + constant learning rate `0.01` + 1 local epoch + 50 rounds，其 `3 x 5` balanced validation Macro-F1 为 `0.4645`；communication-efficient supplementary 为 BatchNorm + SGD momentum `0.9` + constant learning rate `0.03` + 1 local epoch + 20 rounds；后续方法仅在 matching budget 和 matching seed policy 下比较；
...
8. **已完成（2026-07-16）：** 对 frozen HHAR FedAvg primary setting 完成 `3 splits x 5 model seeds` validation-only characterization，冻结全部 15 个 checkpoints、separate variance reporting 和无 seed selection rule；下一步单独 pre-register evaluation-only locked test；
9. 为每个 RQ 预先定义最终 table/figure、target Macro-F1、normalized communication budgets 和 statistical comparison；
10. 冻结 FedRep split point、Ditto regularization search space、fixed local computation budget 及统一调参预算；
11. 写出完整的 UCI HAR smoke-test protocol 和 CAPTURE-24 primary experiment protocol，包括 input sampling rate、seeds、participation rate、stopping rule、validation rule 和 resource cap；
12. 完成以上 specification 工作后再开始实现，不在实现过程中随意扩大研究范围。
