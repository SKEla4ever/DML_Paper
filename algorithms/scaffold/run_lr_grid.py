#!/usr/bin/env python3
"""Run a small SCAFFOLD learning-rate grid."""
...
DEFAULT_OUTPUT_ROOT = Path("outputs/kuhar_scaffold_lr_grid_v1")
...
def extract_summary(run_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "bytes_per_model_state": communication["bytes_per_model_state"],
        "bytes_per_control_variate": communication["bytes_per_control_variate"],
...
def main() -> None:
    args = parse_args()
    script = Path(__file__).with_name("train_kuhar_scaffold.py")
    args.output_root.mkdir(parents=True, exist_ok=True)
    for index, lr in enumerate(args.lr_values, start=1):
        run_dir = args.output_root / lr_name(lr)
        if (run_dir / "final_metrics.json").exists() and not args.force:
            print(f"[{index}/{len(args.lr_values)}] skip existing {run_dir}")
            continue
        command = [
            sys.executable,
            str(script),
        ...
        print(f"[{index}/{len(args.lr_values)}] run {run_dir}")
