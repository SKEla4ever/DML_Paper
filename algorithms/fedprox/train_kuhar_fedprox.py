#!/usr/bin/env python3
"""FedProx 1D-CNN baseline for the frozen KU-HAR V1 manifest."""
...
DEFAULT_OUTPUT_DIR = Path("outputs/kuhar_fedprox_v1")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train a neural FedProx 1D-CNN baseline on KU-HAR frozen V1."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.01,
        help="FedProx proximal coefficient. mu=0 recovers FedAvg local training.",
    )
...
def proximal_penalty(model, global_parameter_state: dict[str, object], device) -> object:
    penalty = None
    for name, parameter in model.named_parameters():
        reference = global_parameter_state[name].to(device)
        term = (parameter - reference).pow(2).sum()
        penalty = term if penalty is None else penalty + term
    if penalty is None:
        return base.torch.tensor(0.0, device=device)
    return penalty


def train_local_fedprox_model(
    ...
    global_parameter_state = {
        name: global_state[name].detach().clone()
    ...
    for _epoch in range(args.local_epochs):
        ...
        for start in range(0, len(shuffled), args.batch_size):
            ...
            if args.mu:
                loss = loss + 0.5 * args.mu * proximal_penalty(
                    model, global_parameter_state, device
                )
            loss.backward()
            optimizer.step()
