...
DEFAULT_OUTPUT_ROOT = Path("outputs/kuhar_ditto_lambda_grid_v1")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Ditto lambda values to sweep.",
    )
...
def lambda_name(value: float) -> str:
    return "lambda" + str(value).replace(".", "p")
...
def extract_summary(run_dir: Path) -> dict[str, object]:
    ...
    return {
        ...
        "global_local_epochs": config["global_local_epochs"],
        "personal_local_epochs": config["personal_local_epochs"],
...
def main() -> None:
    args = parse_args()
    script = Path(__file__).with_name("train_kuhar_ditto.py")
    args.output_root.mkdir(parents=True, exist_ok=True)
    for index, ditto_lambda in enumerate(args.lambda_values, start=1):
        run_dir = args.output_root / lambda_name(ditto_lambda)
        if (run_dir / "final_metrics.json").exists() and not args.force:
            print(f"[{index}/{len(args.lambda_values)}] skip existing {run_dir}")
            continue
        command = [
            sys.executable,
            str(script),
            ...
            "--global-local-epochs",
            str(args.global_local_epochs),
            "--personal-local-epochs",
            str(args.personal_local_epochs),
            "--batch-size",
            str(args.batch_size),
            "--optimizer",
            "adam",
        ...
        print(f"[{index}/{len(args.lambda_values)}] run {run_dir}")
